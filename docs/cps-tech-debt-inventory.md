...
| CPS-TD-014 | AI 规则文档存在被错误编码读取/写入后腐化的治理风险 | P0 | AI 规则读取、代码生成约束、Pre-PR 自查 | `AGENTS.md:409`；`AGENTS.md:411`；`AGENTS.md:413`；`AGENTS.md:501`；`agent_improvement/memory/MEMORY.md:1`；`docs/e2e-agent-issue-workflow.md:1` | 当前两份被点名文档可 UTF-8 解码，但项目已记录 PowerShell 读写中文文件导致不可逆腐化的历史；一旦 AI 读取乱码规则，后续生成代码会绕开真实约束 | 立即建立并执行文档 UTF-8 校验；每次规则/文档改动后都验证 | 否 | `python -c` 读取关键规则文档并 `decode('utf-8')`，扫描替换字符和常见 mojibake 标记 |
...
| CPS-TD-018 | 返利结算“查记录后插入”缺少唯一键/锁，存在并发重复入账窗口 | P1 | 返利结算、重复流水、账户余额 | `backend/qiji-module-cps/qiji-module-cps-biz/src/main/java/com/qiji/cps/module/cps/service/rebate/CpsRebateSettleServiceImpl.java:77`；`backend/qiji-module-cps/qiji-module-cps-biz/src/main/java/com/qiji/cps/module/cps/service/rebate/CpsRebateSettleServiceImpl.java:111`；`backend/qiji-module-cps/qiji-module-cps-biz/src/main/java/com/qiji/cps/module/cps/dal/mysql/order/CpsOrderMapper.java:45`；`backend/sql/module/cps-all-in-one.sql:176`；`backend/sql/module/cps-all-in-one.sql:178` | 当前仅先查已有返利记录再插入，SQL 只见普通 `idx_order_id`，未见 `(tenant_id, order_id, rebate_type)` 唯一约束；并发 Job/手动结算可能双写流水并重复加余额 | 下一次修返利结算或补偿 Job 时处理 | 否 | 并发调用同一订单结算，断言只生成一条 REBATE 流水且账户只加一次 |
