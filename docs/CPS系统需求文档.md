...
> **项目名称**: CPS联盟返利系统  
> **基础框架**: ruoyi-vue-pro (AgenticCPS快速开发平台)
> **版本**: v1.0  
> **日期**: 2026-03-19  
...
返利工具箱面向运营、客服和社群分发人员，把万能转链、口令解析、归属检测、优惠券查询、返利商品广场、淘礼金计划、推广文案编辑和批量复制集中到一个后台工作台。首版仅建设管理后台，不新增用户端页面，不改动返利结算、订单归因、冻结扣减、提现或 Token 兑换链路。
...
| 归属检测 | 按链接、口令或转链记录 ID 检查会员、推广位和平台是否匹配 |
| 优惠券查询 | 复用商品广场有券搜索，按关键词、商品 ID 或链接返回券额、券后价、佣金和有效期 |
| 商品广场 | 在工具箱内复用商品广场筛选、查看和转链能力，支持从活动中心/商品广场跳转进入 |
| 淘礼金计划 | 生成活动模板、补贴预算测算、风险提示和上线检查清单；不调用真实发放接口 |
...
| 返利工具箱 | `/admin-api/cps/goods/ownership-check` | POST | 检测转链会员、推广位和平台归属 |
| 返利工具箱 | `/admin-api/cps/goods/coupon-query` | POST | 查询有券商品 |
| 返利工具箱 | `/admin-api/cps/goods/cash-gift/plan` | POST | 生成淘礼金计划、预算测算和检查清单 |
...
- 统一商品搜索与多平台比价
- 推广链接生成功能
- 管理后台返利工具箱、口令解析、归属检测、优惠券查询、批量转链、淘礼金计划和推广文案编辑
