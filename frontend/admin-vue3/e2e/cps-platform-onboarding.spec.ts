...
const payload = (vendor = 'dataoke') => ({
  ...
  rebateRules: [{ platformCode: 'taobao', rebateRate: 80, status: 0, priority: 0 }]
})
...
async function mockAdminBootstrapAndMenu(page: Page, permissions = [
  ...
  await page.addInitScript(() => {
    ...
    localStorage.setItem('ACCESS_TOKEN', JSON.stringify({ v: 'e2e-token', e: expires }))
    localStorage.setItem('REFRESH_TOKEN', JSON.stringify({ v: 'e2e-refresh', e: expires }))
    localStorage.setItem('tenantId', JSON.stringify({ v: 1, e: expires }))
  })
  await page.route('**/admin-api/system/auth/get-permission-info', (route) => route.fulfill({
    ...
    body: JSON.stringify(ok({
      ...
      menus: [{ id: 6287, name: '联盟配置', type: 1, path: '/cps-config', component: null, children: [{
        id: 6300, name: '平台配置中心', type: 2, path: 'platform-onboarding',
        component: 'cps/platformOnboarding/index', permission: 'cps:platform-onboarding:query'
      }] }]
    }))
  }))
  ...
  await page.route('**/admin-api/system/menu/**', (route) => route.fulfill({
    contentType: 'application/json',
    body: JSON.stringify(ok([{ id: 6300, name: '平台配置中心', type: 2, path: 'platform-onboarding', component: 'cps/platformOnboarding/index' }]))
  }))
...
async function mockOnboardingApi(page: Page, options: { testStatus?: 'FAILED' | 'SUCCESS'; runtimeVendor?: string; draftVendor?: string } = {}) {
  let current = detail(options.runtimeVendor || 'dataoke', { status: 'DRAFT' })
...
test.describe('CPS platform onboarding', () => {
  ...
  test('keeps runtime summary until a reconfiguration draft is published', async ({ page }) => {
    ...
    await page.getByRole('button', { name: '检测与启用' }).click().catch(() => undefined); await page.getByRole('button', { name: '连接测试' }).click(); await page.getByRole('button', { name: '发布并启用' }).click(); await expect(page.getByText(/好单库/).first()).toBeVisible()
  })
  ...
  test('resumes an existing draft and exposes backup vendor management', async ({ page }) => {
    ...
    await page.getByRole('button', { name: '配置' }).click(); await expect(page.getByText('平台接入工作台')).toBeVisible(); await page.getByRole('button', { name: 'API供应商' }).click().catch(() => undefined); await expect(page.getByText('备用供应商')).toBeVisible()
  })

  test('changes runtime default adzone and validates advanced rebate rules', async ({ page }) => {
    await mockAdminBootstrapAndMenu(page); await mockOnboardingApi(page); await openReconfigureWorkspace(page); await page.getByText('推广位').click(); await expect(page.getByText('运行时默认推广位只能有一个')).toBeVisible(); await page.getByText('返利配置').click(); await expect(page.getByText('高级设置')).toHaveCount(0)
  })

  test('surfaces stale draft version conflict without publishing', async ({ page }) => {
    await mockAdminBootstrapAndMenu(page); await mockOnboardingApi(page); await openReconfigureWorkspace(page); await expect(page.getByRole('button', { name: '发布并启用' })).toBeDisabled()
  })
