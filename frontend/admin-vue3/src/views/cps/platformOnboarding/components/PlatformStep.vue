  ...
  <el-form ref="formRef" :model="draft.platform" label-width="110px">
    ...
    <el-form-item label="平台编码" prop="platformCode" required>
      <el-select v-model="draft.platform.platformCode" :disabled="mode === 'RECONFIGURE'" filterable @change="onPlatformChange">
        <el-option v-for="item in capabilities" :key="item.platformCode" :label="getPlatformLabel(item.platformCode, item.platformName)" :value="item.platformCode" />
      </el-select>
    </el-form-item>
...
import { getPlatformLabel } from '@/api/cps/apiVendor'
...
const onPlatformChange = (code: string) => {
  ...
  if (capability && !props.draft.platform.platformName) props.draft.platform.platformName = getPlatformLabel(code, capability.platformName)
}
