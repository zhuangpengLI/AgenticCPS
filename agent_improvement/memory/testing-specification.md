...
## 14. CPS Human-in-the-loop 测试 SOP

> CPS 需求默认采用“人定范围和风险，AI 辅助扫描与生成”的测试方式。AI 不直接决定最终测试范围，最终风险等级和覆盖矩阵由研发确认。

### 14.1 测试范围建立

- 人负责确认最终范围：本次需求涉及哪些 Controller、MCP Tool、Service、Job、Client、Mapper。
- AI 负责从代码改动、调用链和接口入口中生成候选清单。
- 候选清单必须标注：入口、核心 Service、数据表、外部平台、是否影响资金/订单/权限。

### 14.2 风险分级

| 等级 | 判断标准 | 最低测试要求 |
|------|----------|--------------|
| P0 | 资金、返利、提现、订单状态、租户隔离、权限绕过 | 单元测试 + DB 测试 + 关键接口/E2E 或集成验证 |
| P1 | 平台适配、搜索转链、MCP Tool、统计口径 | 单元测试 + Mock 平台响应 + 关键异常路径 |
| P2 | 后台展示、字段调整、非核心查询 | 单元测试或接口级验证 |

AI 可以建议等级，但不能自行降低风险等级。涉及金额、订单、提现、租户隔离时默认不低于 P0。

### 14.3 CPS 覆盖矩阵

每个 CPS 改动至少生成以下覆盖矩阵，并在 PR/交付说明中列出未覆盖原因：

| 维度 | 必查项 |
|------|--------|
| 数据范围 | tenantId、deleted、memberId、平台编码、时间范围 |
| 金额单位 | 商品价、佣金、返利、提现、冻结金额是否均为分 |
| 状态流转 | 正常推进、重复同步、退款、失效、回滚/补偿 |
| 平台差异 | 正常响应、空结果、错误码、签名失败、限流/超时 |
| MCP 安全 | API Key、memberId 上下文、访问日志、错误返回、参数脱敏 |
| 性能 | 分页、索引条件、N+1、批量同步大小、超时设置 |
...
- 本次改动的最小失败/成功测试是什么？
- 哪些路径由自动化测试覆盖，哪些只做了手工验证？
- 是否运行了相关后端 `mvn test -Dtest=...`？
- 是否涉及前端页面，是否运行 `pnpm ts:check` 或 E2E？
- 是否存在因第三方平台不可用而跳过的测试？跳过时是否有 Mock 或替代验证？
- P0/P1 风险是否已由人确认测试深度？
