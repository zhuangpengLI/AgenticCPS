package com.qiji.cps.module.infra.service.codegen.inner;
...
import com.qiji.cps.framework.apilog.core.annotation.ApiAccessLog;
import com.qiji.cps.framework.apilog.core.enums.OperateTypeEnum;
...
import com.qiji.cps.framework.common.util.date.DateUtils;
import com.qiji.cps.framework.common.util.date.LocalDateTimeUtils;
...
import com.qiji.cps.framework.excel.core.annotations.DictFormat;
import com.qiji.cps.framework.excel.core.convert.DictConvert;
import com.qiji.cps.framework.excel.core.util.ExcelUtils;
import com.qiji.cps.framework.mybatis.core.dataobject.BaseDO;
import com.qiji.cps.framework.mybatis.core.mapper.BaseMapperX;
import com.qiji.cps.framework.mybatis.core.query.LambdaQueryWrapperX;
import com.qiji.cps.module.infra.dal.dataobject.codegen.CodegenColumnDO;
import com.qiji.cps.module.infra.dal.dataobject.codegen.CodegenTableDO;
import com.qiji.cps.module.infra.enums.codegen.CodegenFrontTypeEnum;
import com.qiji.cps.module.infra.enums.codegen.CodegenSceneEnum;
import com.qiji.cps.module.infra.enums.codegen.CodegenTemplateTypeEnum;
import com.qiji.cps.module.infra.enums.codegen.CodegenVOTypeEnum;
import com.qiji.cps.module.infra.framework.codegen.config.CodegenProperties;
...
public class CodegenEngine {
    ...
    /**
     * 是否为 qiji-cloud 项目，用于解决 Boot 和 Cloud 的 api 模块兼容性问题
     *
     * true  - 需要有 qiji-module-xxx-api 模块
    ...
    public CodegenEngine() {
        ...
        this.cloudEnable = ClassUtils.isPresent("com.qiji.cps.module.infra.framework.rpc.config.RpcConfiguration",
    ...
    private Map<String, String> getTemplates(Integer frontType) {
        ...
        if (Boolean.FALSE.equals(cloudEnable)) {
            SERVER_TEMPLATES.forEach((templatePath, filePath) -> {
                filePath = StrUtil.replace(filePath, "/qiji-module-${table.moduleName}-api", "");
                filePath = StrUtil.replace(filePath, "/qiji-module-${table.moduleName}-server", "");
    ...
    private static String javaModuleFilePath(String path, String module, String src) {
        return "qiji-module-${table.moduleName}/" + // 顶级模块
                "qiji-module-${table.moduleName}-" + module + "/" + // 子模块
                "src/" + src + "/java/${basePackage}/module/${table.moduleName}/" + path + ".java";
    }

    private static String mapperXmlFilePath() {
        return "qiji-module-${table.moduleName}/" + // 顶级模块
                "qiji-module-${table.moduleName}-server/" + // 子模块
    ...
    private static String vueFilePath(String path) {
        return "qiji-ui-${sceneEnum.basePackage}-vue2/" + // 顶级目录
                "src/" + path;
    }
    ...
    private static String vue3FilePath(String path) {
        return "qiji-ui-${sceneEnum.basePackage}-vue3/" + // 顶级目录
                "src/" + path;
    }
    ...
    private static String vue3UniappFilePath(String path) {
        return "qiji-ui-${sceneEnum.basePackage}-uniapp/" + // 顶级目录
                "src/" + path;
    }

    private static String vue3VbenFilePath(String path) {
        return "qiji-ui-${sceneEnum.basePackage}-vben/" + // 顶级目录
                "src/" + path;
    }
