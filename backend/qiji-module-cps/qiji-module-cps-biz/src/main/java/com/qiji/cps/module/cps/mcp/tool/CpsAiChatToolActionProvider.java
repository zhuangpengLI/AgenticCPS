...
public class CpsAiChatToolActionProvider implements AiChatToolActionProvider {
    ...
    private static final List<AiChatToolAction> ACTIONS = List.of(
            ...
            action("FIND_RESONANCE_GOODS", "cps_find_resonance_goods", "共振选品", GROUP_PROMOTION,
                    AiChatToolRiskLevel.READ_ONLY, AiChatToolInteractionType.FORM,
                    "正在聚合榜单和搜索候选", "共振选品分析完成",
                    "从综合搜索、销量、高佣和热销候选中找{{keyword}}，券后价 {{priceMin}} 至 {{priceMax}} 元，"
                            + "佣金不低于 {{commissionRate}}%，月销量不低于 {{monthSales}}，返回 {{limit}} 件；"
                            + "只做选品分析，不要生成推广链接。",
                    text("keyword", "商品关键词", true, "例如：抽纸、儿童牛奶", null),
                    number("priceMin", "最低券后价（元）", false, ""),
                    number("priceMax", "最高券后价（元）", false, "100"),
                    number("commissionRate", "最低佣金比例（%）", false, "15"),
                    number("monthSales", "最低月销量", false, "1000"),
                    number("limit", "返回数量", true, "10")),
            action("FIND_ALTERNATIVES", "cps_find_alternatives", "高佣替代品", GROUP_PROMOTION,
                    AiChatToolRiskLevel.READ_ONLY, AiChatToolInteractionType.FORM,
                    "正在寻找高佣替代品", "高佣替代品分析完成",
                    "为{{keyword}}找价格相近、佣金更优的替代品；原商品券后价 {{referencePrice}} 元，"
                            + "原商品佣金比例 {{referenceCommissionRate}}%，原商品预估佣金 {{referenceCommissionAmount}} 元，"
                            + "可接受最高溢价 {{maxPricePremiumPercent}}%，佣金不低于 {{commissionRate}}%，"
                            + "月销量不低于 {{monthSales}}，返回 {{limit}} 件；只做选品分析，不要生成推广链接。",
                    text("keyword", "商品关键词", true, "例如：无线降噪耳机", null),
                    number("referencePrice", "原商品券后价（元）", false, ""),
                    number("referenceCommissionRate", "原商品佣金比例（%）", false, ""),
                    number("referenceCommissionAmount", "原商品预估佣金（元）", false, ""),
                    number("maxPricePremiumPercent", "最高溢价（%）", false, "20"),
                    number("commissionRate", "最低佣金比例（%）", false, "20"),
                    number("monthSales", "最低月销量", false, "1000"),
                    number("limit", "返回数量", true, "10")),
            action("ANALYZE_GOODS_DETAIL", "cps_analyze_goods_detail", "商品深度分析", GROUP_GOODS,
                    AiChatToolRiskLevel.READ_ONLY, AiChatToolInteractionType.FORM,
                    "正在分析商品价格、优惠和收益结构", "商品深度分析完成",
                    "分析{{keyword}}的价格带、优惠券、佣金、销量和平台覆盖；券后价范围 {{priceMin}} 至 {{priceMax}} 元，"
                            + "最低月销量 {{monthSales}}，返回 {{limit}} 个候选；只做运营分析，不要生成推广链接。",
                    text("keyword", "商品关键词", true, "例如：无线降噪耳机", null),
                    number("priceMin", "最低券后价（元）", false, ""),
                    number("priceMax", "最高券后价（元）", false, ""),
                    number("monthSales", "最低月销量", false, ""),
                    number("limit", "分析候选数", true, "20")),
            ...
            action("ANALYZE_ORDER_PROFILE", "cps_analyze_order_profile", "成交画像", GROUP_ACCOUNT,
                    AiChatToolRiskLevel.READ_ONLY, AiChatToolInteractionType.FORM,
                    "正在分析成交画像", "成交画像分析完成",
                    "分析当前登录会员最近 {{days}} 天的成交画像，排除商品 ID：{{excludeItemIds}}；"
                            + "只返回聚合证据，不推断缺少数据支撑的人群属性。",
                    number("days", "最近天数", true, "30"),
                    text("excludeItemIds", "排除商品 ID", false, "多个 ID 用逗号分隔，可不填", "")),
            action("ANALYZE_ORDER_TREND", "cps_analyze_order_trend", "成交趋势", GROUP_ACCOUNT,
                    AiChatToolRiskLevel.READ_ONLY, AiChatToolInteractionType.FORM,
                    "正在分析成交趋势", "成交趋势分析完成",
                    "分析当前登录会员最近 {{days}} 天的成交、客单价与返利趋势，按 {{granularity}} 聚合；"
                            + "只做订单运营复盘，不修改订单、归因或返利资产。",
                    number("days", "最近天数", true, "30"),
                    select("granularity", "趋势粒度", true, "daily",
                            option("按天", "daily"), option("按周", "weekly"))),
