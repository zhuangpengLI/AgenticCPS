...
import com.qiji.cps.module.report.controller.admin.goview.vo.data.GoViewDataRespVO;
...
public class GoViewDataServiceImplTest extends BaseDbUnitTest {
    ...
    public void testGetDataBySQL() {
        ...
        when(sqlRowSet.getObject("name")).thenReturn("AgenticCPS源码").thenReturn("AgenticCPS");
        ...
        assertEquals("AgenticCPS源码", dataBySQL.getSource().get(0).get("name"));
        assertEquals(2, dataBySQL.getSource().get(1).size());
        assertEquals(2L, dataBySQL.getSource().get(1).get("id"));
        assertEquals("AgenticCPS", dataBySQL.getSource().get(1).get("name"));
    }
