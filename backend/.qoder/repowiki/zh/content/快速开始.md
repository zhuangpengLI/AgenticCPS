...
## 项目结构
- 后端采用多模块 Maven 结构，核心模块包括 yudao-server（服务端）、yudao-module-cps（CPS业务模块）等。
- 前端包含 Vue3 管理后台与 uni-app 移动端工程（位于 yudao-ui 目录下，详见 README）。
- 项目使用 Spring Boot 3.x + MyBatis Plus，数据库支持 MySQL、Oracle、PostgreSQL、SQL Server、达梦、OpenGauss 等，缓存使用 Redis。

```mermaid
graph TB
A["根项目 pom.xml<br/>多模块聚合"] --> B["yudao-server<br/>后端服务"]
A --> C["yudao-module-cps<br/>CPS业务模块"]
A --> D["yudao-framework<br/>框架扩展"]
A --> E["yudao-module-system<br/>系统模块"]
A --> F["yudao-module-member<br/>会员模块"]
A --> G["yudao-module-infra<br/>基础设施模块"]
A --> H["yudao-module-pay<br/>支付模块"]
A --> I["yudao-module-mall<br/>商城模块"]
A --> J["yudao-module-ai<br/>AI模块"]
A --> K["yudao-module-mp<br/>微信公众号模块"]
A --> L["yudao-module-report<br/>报表模块"]
```
...
## 核心组件
- 后端服务：yudao-server，负责对外提供 REST API、定时任务、监控与安全等能力。
- CPS 模块：yudao-module-cps，提供多平台CPS接入、商品搜索比价、返利与提现、MCP AI接口等。
- 基础设施：yudao-framework 与 yudao-module-infra，提供数据源、缓存、定时任务、消息队列、监控等通用能力。
- 系统模块：yudao-module-system，提供用户、角色、菜单、部门、租户等系统基础能力。
- 会员与支付：yudao-module-member 与 yudao-module-pay，复用会员与支付能力。
...
- 定时任务：Quartz（由 yudao-spring-boot-starter-job 提供）。
- 消息队列：支持 RocketMQ、Kafka、RabbitMQ 等。

**章节来源**
- [pom.xml:34](file://pom.xml#L34)
- [yudao-server/Dockerfile:3](file://yudao-server/Dockerfile#L3)
...
- Quartz 表结构由 yudao-spring-boot-starter-job 提供，可在本地禁用自动配置或手动初始化。
...
**章节来源**
- [yudao-server/src/main/resources/application.yaml:1-353](file://yudao-server/src/main/resources/application.yaml#L1-L353)
- [yudao-server/src/main/resources/application-local.yaml:1-293](file://yudao-server/src/main/resources/application-local.yaml#L1-L293)
...
- 本地启动
  ...
  - 启动 yudao-server（application.yaml 激活 local）。
- 容器化启动
...
**章节来源**
- [yudao-server/src/main/resources/application-local.yaml:161-163](file://yudao-server/src/main/resources/application-local.yaml#L161-L163)
- [yudao-server/src/main/resources/application.yaml:154-166](file://yudao-server/src/main/resources/application.yaml#L154-L166)
...
**章节来源**
- [sql/mysql/ruoyi-vue-pro.sql:1-200](file://sql/mysql/ruoyi-vue-pro.sql#L1-L200)
- [yudao-server/src/main/resources/application-local.yaml:116-118](file://yudao-server/src/main/resources/application-local.yaml#L116-L118)

## 依赖分析
- 构建与运行依赖：Maven、JDK 17、Spring Boot 3.x、MyBatis Plus、Redis、Quartz、消息队列等。
- 模块依赖：yudao-module-cps 依赖 yudao-module-member、yudao-module-pay、yudao-module-system、yudao-module-infra 等模块能力。

```mermaid
graph LR
Root["根 pom.xml"] --> Server["yudao-server"]
...
**章节来源**
- [yudao-server/src/main/resources/application-local.yaml:146-194](file://yudao-server/src/main/resources/application-local.yaml#L146-L194)
- [yudao-server/src/main/resources/application.yaml:90-96](file://yudao-server/src/main/resources/application.yaml#L90-L96)
...
**章节来源**
- [yudao-server/src/main/resources/application.yaml:41-48](file://yudao-server/src/main/resources/application.yaml#L41-L48)
- [yudao-server/src/main/resources/application-local.yaml:93-118](file://yudao-server/src/main/resources/application-local.yaml#L93-L118)
