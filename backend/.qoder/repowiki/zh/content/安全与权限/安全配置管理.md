...
- 安全配置适配器YudaoWebSecurityConfigurerAdapter的职责与实现
- SecurityProperties配置项的含义与使用方法
- 自动配置YudaoSecurityAutoConfiguration的加载顺序与组件注册
...
## 项目结构
围绕安全配置的核心代码位于yudao-framework/yudao-spring-boot-starter-security模块，同时在各业务模块中通过AuthorizeRequestsCustomizer扩展安全规则。

```mermaid
graph TB
subgraph "安全框架"
SP["SecurityProperties<br/>安全配置项"]
ASA["YudaoSecurityAutoConfiguration<br/>自动配置"]
WSCA["YudaoWebSecurityConfigurerAdapter<br/>安全适配器"]
...
subgraph "应用配置"
APP_YAML["application.yaml<br/>yudao.security.* 配置"]
end
APP_YAML --> SP
ASA --> SP
ASA --> TAF
...
- [TransmittableThreadLocalSecurityContextHolderStrategy.java:15-49](file://yudao-framework/yudao-spring-boot-starter-security/src/main/java/cn/iocoder/yudao/framework/security/core/context/TransmittableThreadLocalSecurityContextHolderStrategy.java#L15-L49)
- [SecurityConfiguration.java（AI模块）:17-43](file://yudao-module-ai/src/main/java/cn/iocoder/yudao/module/ai/framework/security/config/SecurityConfiguration.java#L17-L43)
- [SecurityConfiguration.java（Infra模块）:12-39](file://yudao-module-infra/src/main/java/cn/iocoder/yudao/module/infra/framework/security/config/SecurityConfiguration.java#L12-L39)
- [SecurityConfiguration.java（Report模块）:12-32](file://yudao-module-report/src/main/java/cn/iocoder/yudao/module/report/framework/security/config/SecurityConfiguration.java#L12-L32)
...
- YudaoSecurityAutoConfiguration：负责注册认证入口、权限不足处理器、PasswordEncoder、Token过滤器、权限服务以及Security上下文策略。
- YudaoWebSecurityConfigurerAdapter：定义安全过滤器链、拦截规则、异常处理、基于注解的免登录URL收集等。
...
```mermaid
graph TB
A["客户端请求"] --> B["YudaoWebSecurityConfigurerAdapter<br/>过滤器链"]
...
G["YudaoSecurityAutoConfiguration<br/>组件注册"] --> C
G --> E
...
### 安全配置适配器：YudaoWebSecurityConfigurerAdapter
- 责任边界
  - 定义SecurityFilterChain，禁用CSRF、禁用Session、允许跨域、配置异常处理器
  - 收集@PermitAll注解的URL并加入免登录白名单
  - 合并yudao.security.permit-all-urls配置与自定义AuthorizeRequestsCustomizer
  - 任何未匹配请求均需认证
...
### 自动配置：YudaoSecurityAutoConfiguration
- 组件注册
...
YudaoSecurityAutoConfiguration --> TokenAuthenticationFilter : "注册"
YudaoSecurityAutoConfiguration --> SecurityFrameworkServiceImpl : "注册"
YudaoSecurityAutoConfiguration --> TransmittableThreadLocalSecurityContextHolderStrategy : "设置策略"
```
...
- [SecurityFrameworkUtils.java:41-54](file://yudao-framework/yudao-spring-boot-starter-security/src/main/java/cn/iocoder/yudao/framework/security/core/util/SecurityFrameworkUtils.java#L41-L54)
...
- 机制
  - 通过实现AuthorizeRequestsCustomizer并注入容器，即可在YudaoWebSecurityConfigurerAdapter中被扫描并应用
- 示例
...
## 依赖分析
- 组件耦合
  - YudaoSecurityAutoConfiguration集中注册核心组件，降低适配器与外部依赖的耦合
...
ASA["YudaoSecurityAutoConfiguration"] --> SP
...
WSCA["YudaoWebSecurityConfigurerAdapter"] --> TAF
WSCA --> SP
...
- mock模式无效
  - 确认application.yaml中yudao.security.mockEnable=true且mockSecret配置正确
- 自定义放行规则不生效
  - 检查AuthorizeRequestsCustomizer是否被容器发现并注入
  - 确认YudaoWebSecurityConfigurerAdapter的自定义规则合并逻辑

**章节来源**
- [TokenAuthenticationFilter.java:40-117](file://yudao-framework/yudao-spring-boot-starter-security/src/main/java/cn/iocoder/yudao/framework/security/core/filter/TokenAuthenticationFilter.java#L40-L117)
- [SecurityFrameworkServiceImpl.java:24-82](file://yudao-framework/yudao-spring-boot-starter-security/src/main/java/cn/iocoder/yudao/framework/security/core/service/SecurityFrameworkServiceImpl.java#L24-L82)
- [YudaoWebSecurityConfigurerAdapter.java:143-148](file://yudao-framework/yudao-spring-boot-starter-security/src/main/java/cn/iocoder/yudao/framework/security/config/YudaoWebSecurityConfigurerAdapter.java#L143-L148)
- [application.yaml:272-281](file://yudao-server/src/main/resources/application.yaml#L272-L281)

## 结论
AgenticCPS的安全配置以YudaoSecurityAutoConfiguration与YudaoWebSecurityConfigurerAdapter为核心，通过SecurityProperties集中管理配置，借助TokenAuthenticationFilter与SecurityFrameworkService实现统一的认证与授权。业务模块通过AuthorizeRequestsCustomizer灵活扩展放行规则，配合TransmittableThreadLocal策略保障异步场景下的上下文一致性。生产环境建议严格关闭mock模式、合理设置BCrypt复杂度与放行规则，持续监控权限校验链路与Token解析性能。
...
- 配置模板
  - application.yaml中建议显式配置yudao.security.*相关项
