...
- 模块聚合：yudao-module-cps（父 POM），负责模块声明与聚合。
- 业务实现：yudao-module-cps-biz（子模块），包含控制器、服务、数据访问对象、枚举、配置与定时任务等。

```mermaid
graph TB
subgraph "CPS 模块"
A["yudao-module-cps<br/>父POM"]
B["yudao-module-cps-biz<br/>业务实现"]
end
A --> B
```

图表来源
- [yudao-module-cps/pom.xml:1-25](file://yudao-module-cps/pom.xml#L1-L25)
- [yudao-module-cps-biz/pom.xml:1-122](file://yudao-module-cps/yudao-module-cps-biz/pom.xml#L1-L122)
...
M --> DB["数据库<br/>yudao_cps_order 等表"]
...
- 数据库表
  - 由 cps-schema.sql 定义，包含 yudao_cps_order 等核心表。
...
## 依赖分析
- 模块依赖
  - yudao-module-cps-biz 依赖 system/member/infra 模块，以及 yudao-spring-boot-starter-* 组件（安全、验证、MyBatis、Redis、作业调度、Excel 等）。
- 运行时依赖
  - OkHttp3 客户端用于平台对接；RestTemplate 通过 OkHttp3ClientHttpRequestFactory 注入；线程池用于多平台并行搜索。

```mermaid
graph LR
Biz["yudao-module-cps-biz"] --> Sys["yudao-module-system"]
Biz --> Mem["yudao-module-member"]
Biz --> Infra["yudao-module-infra"]
