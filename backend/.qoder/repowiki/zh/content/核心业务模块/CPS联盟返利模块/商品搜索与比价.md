...
## 项目结构
CPS模块采用分层与模块化组织，核心位于 yudao-module-cps/yudao-module-cps-biz，包含客户端适配层、服务层、作业调度、配置与枚举等。模块父工程 yudao-module-cps 聚合子模块，便于统一构建与发布。

```mermaid
graph TB
A["yudao-module-cps<br/>父工程"] --> B["yudao-module-cps-biz<br/>业务实现模块"]
...
```mermaid
graph LR
P["yudao-module-cps/pom.xml"] --> B["yudao-module-cps-biz"]
