...
- [pom.xml](file://qiji-server/pom.xml)
...
- 后端服务：qiji-server，使用Spring Boot打包为可执行JAR，通过Dockerfile构建镜像并暴露48080端口。
- 数据库：MySQL 8与Redis 6-alpine，通过docker-compose统一编排。
- 前端管理端：qiji-ui-admin，独立容器，通过Nginx提供静态资源与API反向代理。
- 辅助工具：提供多数据库环境的docker-compose，便于本地开发与测试。

```mermaid
graph TB
subgraph "容器编排"
S["后端服务<br/>qiji-server"]
M["数据库<br/>MySQL 8"]
R["缓存<br/>Redis 6-alpine"]
A["前端管理端<br/>qiji-ui-admin"]
end
A --> |"反向代理 /prod-api"| S
S --> M
S --> R
...
participant B as "后端(qiji-server)"
...
- 工作目录与文件复制
  - 在镜像内创建/qiji-server目录并设置为工作目录，将打包好的JAR复制到app.jar。
- 环境变量
...
Base --> WorkDir["创建工作目录<br/>/qiji-server"]
...
- 服务定义
  ...
  - server：后端服务，基于qiji-server目录构建，端口48080，传递JAVA_OPTS与ARGS。
  - admin：前端管理端，基于qiji-ui-admin构建，端口80，反向代理后端API。
- 网络与卷
...
- Docker镜像构建
  - docker-compose基于qiji-server/Dockerfile构建后端镜像。
  - docker-compose基于qiji-ui-admin/Dockerfile构建前端镜像。
- 启动服务
...
- [pom.xml:117-135](file://qiji-server/pom.xml#L117-L135)

章节来源
- [Docker-HOWTO.md:1-50](file://script/docker/Docker-HOWTO.md#L1-L50)
- [pom.xml:1-138](file://qiji-server/pom.xml#L1-L138)
