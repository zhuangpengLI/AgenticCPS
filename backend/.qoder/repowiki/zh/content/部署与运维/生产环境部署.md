...
- [org.springframework.boot.autoconfigure.AutoConfiguration.imports](file://qiji-framework/qiji-spring-boot-starter-monitor/src/main/resources/META-INF/spring/org.springframework.boot.autoconfigure.AutoConfiguration.imports)
</cite>
...
LB["负载均衡/多实例"]
subgraph "应用层"
Srv["qiji-server 应用容器"]
Admin["qiji-admin 前端容器"]
end
subgraph "数据层"
MySQL["MySQL 主库/从库"]
Redis["Redis 缓存"]
...
## 核心组件
- 应用服务：qiji-server，提供REST API、定时任务、消息队列集成、WebSocket推送、AI向量存储与多模型接入等能力。
...
- [application-dev.yaml:1-212](file://qiji-server/src/main/resources/application-dev.yaml#L1-L212)
...
Nginx --> LB["负载均衡/多实例"]
LB --> App1["qiji-server 实例1"]
LB --> App2["qiji-server 实例2"]
LB --> AppN["qiji-server 实例N"]
App1 --> DB["MySQL 主库/从库"]
App2 --> DB
AppN --> DB
App1 --> Cache["Redis"]
App2 --> Cache
AppN --> Cache
AdminFE["qiji-admin 前端"] --> Nginx
```

图表来源
- [docker-compose.yml:29-78](file://script/docker/docker-compose.yml#L29-L78)
- [application-dev.yaml:48-66](file://qiji-server/src/main/resources/application-dev.yaml#L48-L66)
...
### JDK与容器化部署
- 容器镜像：基于eclipse-temurin:21-jre，工作目录/qiji-server，暴露48080端口。
...
图表来源
- [application-dev.yaml:32-57](file://qiji-server/src/main/resources/application-dev.yaml#L32-L57)
...
章节来源
- [application-dev.yaml:59-66](file://qiji-server/src/main/resources/application-dev.yaml#L59-L66)
- [application.yaml:90-96](file://qiji-server/src/main/resources/application.yaml#L90-L96)

### Nginx反向代理
- 作用：统一入口、静态资源分发、HTTPS终止、健康检查与限流。
- 建议：将qiji-admin前端容器映射到80端口，后端qiji-server映射到48080端口；配置域名与证书。
- 负载均衡：后端多实例部署时，Nginx配置upstream指向多个qiji-server实例。
...
participant A as "qiji-server 实例A"
participant B as "qiji-server 实例B"
C->>N : "HTTP 请求"
N->>L : "转发请求"
L->>A : "优先选择健康实例"
alt "实例A不可用"
...
- [WebSocketSessionManagerImpl.java:1-42](file://qiji-framework/qiji-spring-boot-starter-websocket/src/main/java/com.qiji.cps/framework/websocket/core/session/WebSocketSessionManagerImpl.java#L1-L42)
...
- [WebSocketSessionManager.java:1-53](file://qiji-framework/qiji-spring-boot-starter-websocket/src/main/java/com.qiji.cps/framework/websocket/core/session/WebSocketSessionManager.java#L1-L53)
- [WebSocketSessionManagerImpl.java:1-42](file://qiji-framework/qiji-spring-boot-starter-websocket/src/main/java/com.qiji.cps/framework/websocket/core/session/WebSocketSessionManagerImpl.java#L1-L42)
- [WebSocketSessionHandlerDecorator.java:1-37](file://qiji-framework/qiji-spring-boot-starter-websocket/src/main/java/com.qiji.cps/framework/websocket/core/session/WebSocketSessionHandlerDecorator.java#L1-L37)

章节来源
- [WebSocketAuthorizeRequestsCustomizer.java:1-24](file://qiji-framework/qiji-spring-boot-starter-websocket/src/main/java/com.qiji.cps/framework/websocket/core/security/WebSocketAuthorizeRequestsCustomizer.java#L1-L24)
...
```mermaid
graph LR
App["qiji-server"] --> Mon["监控自动配置"]
App --> WS["WebSocket 安全配置"]
App --> Admin["Admin Server 安全链"]
```

图表来源
- [org.springframework.boot.autoconfigure.AutoConfiguration.imports:1-2](file://qiji-framework/qiji-spring-boot-starter-monitor/src/main/resources/META-INF/spring/org.springframework.boot.autoconfigure.AutoConfiguration.imports#L1-L2)
- [WebSocketAuthorizeRequestsCustomizer.java:1-24](file://qiji-framework/qiji-spring-boot-starter-websocket/src/main/java/com.qiji.cps/framework/websocket/core/security/WebSocketAuthorizeRequestsCustomizer.java#L1-L24)
- [AdminServerConfiguration.java:59-80](file://qiji-module-infra/src/main/java/com.qiji.cps/module/infra/framework/monitor/config/AdminServerConfiguration.java#L59-L80)
...
章节来源
- [application-dev.yaml:117-121](file://qiji-server/src/main/resources/application-dev.yaml#L117-L121)
- [application.yaml:98-119](file://qiji-server/src/main/resources/application.yaml#L98-L119)
...
- [application-dev.yaml:124-150](file://qiji-server/src/main/resources/application-dev.yaml#L124-L150)
