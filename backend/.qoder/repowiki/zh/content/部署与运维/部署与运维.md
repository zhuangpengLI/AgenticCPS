...
## 项目结构
AgenticCPS为多模块Maven工程，核心后端位于qiji-server，前端位于qiji-ui-*，容器化与运维脚本集中在script目录。

```mermaid
graph TB
subgraph "后端服务"
S["qiji-server<br/>Spring Boot 应用"]
F["qiji-framework<br/>通用能力模块"]
...
subgraph "前端"
UI1["qiji-ui-admin-vue3"]
UI2["qiji-ui-admin-uniapp"]
UI3["qiji-ui-mall-uniapp"]
end
subgraph "容器与运维"
DC["script/docker/docker-compose.yml"]
DF["qiji-server/Dockerfile"]
...
- docker-compose.yml要点
  ...
  - 卷：持久化MySQL与Redis数据
- 镜像构建与发布
  - 本地构建：在qiji-server目录执行docker build
...
**章节来源**
- [application.yaml:26-31](file://qiji-server/src/main/resources/application.yaml#L26-L31)
...
## 依赖分析
- 模块依赖
  - qiji-server聚合qiji-framework与各业务模块
  - qiji-framework提供监控、消息队列、安全、Redis、MyBatis等通用能力
- 运行时依赖
...
```mermaid
graph LR
P["pom.xml<br/>聚合模块"] --> S["qiji-server"]
P --> F["qiji-framework"]
P --> M1["qiji-module-system"]
P --> M2["qiji-module-infra"]
P --> M3["qiji-module-member"]
P --> M4["qiji-module-pay"]
P --> M5["qiji-module-mall"]
P --> M6["qiji-module-ai"]
P --> M7["qiji-module-cps"]
S --> F
S --> M1
S --> M2
S --> M3
...
**章节来源**
- [Dockerfile:11-18](file://qiji-server/Dockerfile#L11-L18)
- [application-dev.yaml:13-58](file://qiji-server/src/main/resources/application-dev.yaml#L13-L58)
- [application.yaml:26-31](file://qiji-server/src/main/resources/application.yaml#L26-L31)

## 故障排除指南
- 启动失败
  - 检查QijiServerApplication启动类与扫描包配置
...
**章节来源**
- [QijiServerApplication.java:16-32](file://qiji-server/src/main/java/com.qiji.cps/server/QijiServerApplication.java#L16-L32)
- [application.yaml:5-10](file://qiji-server/src/main/resources/application.yaml#L5-L10)
