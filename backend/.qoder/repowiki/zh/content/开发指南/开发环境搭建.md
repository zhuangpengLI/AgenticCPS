...
- qiji-dependencies：统一依赖版本管理
- qiji-framework：框架扩展与公共能力
- qiji-server：后端服务聚合模块
- qiji-module-*：各业务模块（如system、infra、member、pay、mall、ai、mp、report、cps等）
- qiji-ui-*：前端UI（admin-vue3、admin-uniapp、mall-uniapp）

```mermaid
graph TB
A["根工程<br/>pom.xml"] --> B["qiji-dependencies<br/>统一依赖版本"]
A --> C["qiji-framework<br/>框架扩展"]
A --> D["qiji-server<br/>服务聚合"]
A --> E["qiji-module-system"]
A --> F["qiji-module-infra"]
A --> G["qiji-module-member"]
A --> H["qiji-module-pay"]
A --> I["qiji-module-mall"]
A --> J["qiji-module-ai"]
A --> K["qiji-module-mp"]
A --> L["qiji-module-report"]
A --> M["qiji-module-cps"]
```
...
## 架构总览
后端服务通过qiji-server聚合各业务模块，提供RESTful API给前端（admin-vue3、admin-uniapp、mall-uniapp）。系统支持多数据源、分布式锁、定时任务、消息队列、监控与链路追踪等基础设施能力。

```mermaid
graph TB
subgraph "后端服务"
S["qiji-server<br/>聚合模块"]
M1["qiji-module-system"]
M2["qiji-module-infra"]
M3["qiji-module-member"]
M4["qiji-module-pay"]
M5["qiji-module-mall"]
M6["qiji-module-ai"]
M7["qiji-module-mp"]
M8["qiji-module-report"]
M9["qiji-module-cps"]
end
subgraph "基础设施"
DB["MySQL 8.0+"]
...
图表来源
- [qiji-server/pom.xml:23-99](file://qiji-server/pom.xml#L23-L99)
- [application-dev.yaml:5-66](file://qiji-server/src/main/resources/application-dev.yaml#L5-L66)
- [application-local.yaml:5-87](file://qiji-server/src/main/resources/application-local.yaml#L5-L87)
...
- [qiji-dependencies/pom.xml:458-477](file://qiji-dependencies/pom.xml#L458-L477)
...
章节来源
- [application-dev.yaml:50-57](file://qiji-server/src/main/resources/application-dev.yaml#L50-L57)
- [application-local.yaml:50-86](file://qiji-server/src/main/resources/application-local.yaml#L50-L86)
...
- 关键配置项说明
  ...
  - qiji自定义配置：qiji.pay、qiji.cps、qiji.security等

章节来源
- [application-dev.yaml:1-212](file://qiji-server/src/main/resources/application-dev.yaml#L1-L212)
- [application-local.yaml:1-293](file://qiji-server/src/main/resources/application-local.yaml#L1-L293)

### 开发环境验证
- 项目编译：mvn clean install（或IDE中执行）
- 数据库连接测试：启动qiji-server，访问Druid监控页面（默认开启）或查看日志
...
- [application-dev.yaml:1-2](file://qiji-server/src/main/resources/application-dev.yaml#L1-L2)
...
- [lombok.config:1-5](file://lombok.config#L1-L5)
- [qiji-dependencies/pom.xml:458-477](file://qiji-dependencies/pom.xml#L458-L477)
- [qiji-dependencies/pom.xml:180-216](file://qiji-dependencies/pom.xml#L180-L216)
- [qiji-dependencies/pom.xml:244-260](file://qiji-dependencies/pom.xml#L244-L260)
- [qiji-dependencies/pom.xml:305-322](file://qiji-dependencies/pom.xml#L305-L322)
- [qiji-dependencies/pom.xml:324-370](file://qiji-dependencies/pom.xml#L324-L370)

## 依赖关系分析
- 依赖管理：qiji-dependencies通过BOM集中管理Spring Boot、MyBatis Plus、Redisson、MapStruct、Lombok等版本
- 构建插件：maven-compiler-plugin、maven-surefire-plugin、flatten-maven-plugin等
- qiji-server聚合各业务模块，按需启用（默认注释的模块可按需取消注释以启用）

```mermaid
graph LR
P["根工程<br/>pom.xml"] --> D["qiji-dependencies<br/>BOM"]
P --> S["qiji-server<br/>聚合模块"]
S --> M1["qiji-module-system"]
S --> M2["qiji-module-infra"]
S --> M3["qiji-module-member"]
S --> M4["qiji-module-pay"]
S --> M5["qiji-module-mall"]
S --> M6["qiji-module-ai"]
S --> M7["qiji-module-mp"]
S --> M8["qiji-module-report"]
S --> M9["qiji-module-cps"]
```
...
章节来源
- [pom.xml:47-57](file://pom.xml#L47-L57)
- [qiji-dependencies/pom.xml:84-101](file://qiji-dependencies/pom.xml#L84-L101)
- [qiji-server/pom.xml:23-99](file://qiji-server/pom.xml#L23-L99)
...
章节来源
- [application-dev.yaml:13-31](file://qiji-server/src/main/resources/application-dev.yaml#L13-L31)
- [application-local.yaml:13-31](file://qiji-server/src/main/resources/application-local.yaml#L13-L31)
...
- 常用命令
  - mvn clean install
  - mvn spring-boot:run（在qiji-server模块）
- 参考文档
  - README.md中的技术栈与模块说明
  - 各模块的入门文档（位于qiji-framework与各模块的资源目录中）