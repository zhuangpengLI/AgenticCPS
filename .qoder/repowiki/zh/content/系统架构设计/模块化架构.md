...
- [backend/yudao-module-report/pom.xml](file://backend/yudao-module-report/pom.xml)
- [backend/yudao-module-mp/pom.xml](file://backend/yudao-module-mp/pom.xml)
- [backend/yudao-server/pom.xml](file://backend/yudao-server/pom.xml)
</cite>

## 更新摘要
**所做更改**
- 新增 yudao-module-cps CPS联盟返利系统模块架构
- 更新模块依赖关系图，包含新的CPS模块
- 补充CPS模块的职责划分和接口规范
- 更新服务器装配配置，包含CPS模块依赖
...
- 业务模块划分（system、member、pay、mall 子模块族、ai、infra、cps 等）
...
- 业务模块按领域拆分，如 system、member、pay、mall（product/promotion/trade/statistics/trade-api）、ai、infra、cps 等
...
Root --> ModCPS["yudao-module-cps<br/>API/Biz"]
ModCPS --> CPSAPI["yudao-module-cps-api"]
ModCPS --> CPSBiz["yudao-module-cps-biz"]
```
...
- 业务层：system、member、pay、mall（product/promotion/trade/statistics/trade-api）、ai、infra、cps 等按领域划分
...
- 设计：作为上层业务模块（member、pay、mall、cps 等）的共同基础
...
### yudao-module-cps：CPS联盟返利系统
- 模块结构：
  - yudao-module-cps-api：CPS模块API层，暴露给其他模块调用
  - yudao-module-cps-biz：CPS模块业务实现层，包含平台配置、推广位管理、订单同步、返利计算、提现管理、MCP AI接口等功能
- 职责与功能：
  - 平台管理：CPS联盟平台配置与管理
  - 推广位管理：推广链接、推广位配置与统计
  - 订单同步：对接第三方平台订单数据同步
  - 返利计算：基于规则的返利计算引擎
  - 提现管理：推广员提现申请与审核流程
  - MCP AI接口：集成AI能力进行智能推荐与数据分析
- 依赖关系：CPS API模块依赖yudao-common和参数校验；CPS Biz模块依赖CPS API、member、pay、system模块，以及各种业务组件和框架starter

**章节来源**
- [backend/yudao-module-cps/pom.xml:17-24](file://backend/yudao-module-cps/pom.xml#L17-L24)
- [backend/yudao-module-cps/yudao-module-cps-api/pom.xml:15-31](file://backend/yudao-module-cps/yudao-module-cps-api/pom.xml#L15-L31)
- [backend/yudao-module-cps/yudao-module-cps-biz/pom.xml:15-93](file://backend/yudao-module-cps/yudao-module-cps-biz/pom.xml#L15-L93)

### yudao-module-report：数据可视化报表
- 职责：基于积木报表实现打印设计、报表设计、图形设计、大屏设计等功能
- 依赖：system模块、业务组件、web安全、mybatis、测试、Excel工具等

**章节来源**
- [backend/yudao-module-report/pom.xml:15-72](file://backend/yudao-module-report/pom.xml#L15-L72)

### yudao-module-mp：微信公众号模块
- 职责：提供微信公众号的账号、菜单、粉丝、标签、消息、自动回复、素材、模板通知、运营数据等功能
- 依赖：system、infra模块、业务组件、安全、验证、mybatis、redis、消息队列、测试、Excel工具、微信SDK等

**章节来源**
- [backend/yudao-module-mp/pom.xml:15-88](file://backend/yudao-module-mp/pom.xml#L15-L88)

### yudao-server：装配与打包容器
- 职责：按需引入 system、infra、member、report、pay、mp、product/promotion/trade/statistics、ai、cps 等模块，通过 spring-boot-maven-plugin 打包为可执行 jar
- 设计：默认注释部分模块依赖，保证编译速度与部署灵活性
- **更新**：已包含 yudao-module-cps-biz 依赖，支持CPS联盟返利系统的完整部署

**章节来源**
- [backend/yudao-server/pom.xml:23-127](file://backend/yudao-server/pom.xml#L23-L127)
- [backend/yudao-server/pom.xml:94-99](file://backend/yudao-server/pom.xml#L94-L99)
...
K --> K1["yudao-module-cps-api"]
K --> K2["yudao-module-cps-biz"]
K2 --> G
K2 --> H
K2 --> E
```
...
- system 与 infra 作为通用基础，被 member、pay、mall、ai、cps 等模块广泛依赖
- **新增**：CPS模块采用API/Biz分层架构，通过yudao-module-cps-api提供对外接口，yudao-module-cps-biz实现具体业务逻辑，避免模块间直接耦合
...
- [backend/yudao-module-mall/yudao-module-trade-api/pom.xml:19-31](file://backend/yudao-module-mall/yudao-module-trade-api/pom.xml#L19-L31)
- [backend/yudao-module-cps/yudao-module-cps-api/pom.xml:15-31](file://backend/yudao-module-cps/yudao-module-cps-api/pom.xml#L15-L31)
- [backend/yudao-module-cps/yudao-module-cps-biz/pom.xml:21-45](file://backend/yudao-module-cps/yudao-module-cps-biz/pom.xml#L21-L45)
- [backend/yudao-module-system/pom.xml:20-122](file://backend/yudao-module-system/pom.xml#L20-L122)
- [backend/yudao-module-member/pom.xml:20-84](file://backend/yudao-module-member/pom.xml#L20-L84)
- [backend/yudao-module-pay/pom.xml:21-81](file://backend/yudao-module-pay/pom.xml#L21-L81)
- [backend/yudao-module-ai/pom.xml:28-262](file://backend/yudao-module-ai/pom.xml#L28-L262)
...
- yudao-module-mall：电商子域（product/promotion/trade/statistics/trade-api）
- yudao-module-ai：大模型与向量检索
- **新增**：yudao-module-cps：CPS联盟返利系统（API/Biz分层）
- yudao-module-report：数据可视化报表
- yudao-module-mp：微信公众号模块
...
- [backend/yudao-module-cps/pom.xml:17-20](file://backend/yudao-module-cps/pom.xml#L17-L20)
- [backend/yudao-module-report/pom.xml:15-18](file://backend/yudao-module-report/pom.xml#L15-L18)
- [backend/yudao-module-mp/pom.xml:15-18](file://backend/yudao-module-mp/pom.xml#L15-L18)

### 模块接口规范
- trade-api：抽取 trade 与 promotion 的公共接口，供双方调用，避免循环依赖
- **新增**：CPS API模块：通过yudao-module-cps-api提供标准化接口，供其他模块调用CPS功能
...
- [backend/yudao-module-cps/yudao-module-cps-api/pom.xml:25-30](file://backend/yudao-module-cps/yudao-module-cps-api/pom.xml#L25-L30)
...
- **更新**：CPS模块采用API/Biz分层设计，减少模块间直接依赖，提高系统可维护性
...
- **新增**：CPS模块启动失败：检查 yudao-module-cps-api 和 yudao-module-cps-biz 的依赖配置，确认 member、pay、system 模块可用
...
- [backend/yudao-module-mall/yudao-module-trade/pom.xml:21-45](file://backend/yudao-module-mall/yudao-module-trade/pom.xml#L21-L45)
- [backend/yudao-module-mall/yudao-module-promotion/pom.xml:22-46](file://backend/yudao-module-mall/yudao-module-promotion/pom.xml#L22-L46)
- [backend/yudao-module-cps/yudao-module-cps-biz/pom.xml:28-45](file://backend/yudao-module-cps/yudao-module-cps-biz/pom.xml#L28-L45)
...
- 明确的职责边界与低耦合
- 可复用的技术/业务组件与统一版本治理
- 清晰的模块间通信契约（如 trade-api 解耦、CPS API/Biz 分层）
- 良好的可维护性与可扩展性，便于按需裁剪与演进
- **新增**：CPS联盟返利系统模块的完整集成，进一步丰富了系统的业务能力
...
- 模块化设计原则
  ...
  - **新增**：API/Biz分层：通过API模块提供标准化接口，Biz模块实现业务逻辑，降低模块间耦合
