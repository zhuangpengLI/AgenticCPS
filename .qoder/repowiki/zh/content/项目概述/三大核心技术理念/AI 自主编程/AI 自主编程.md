...
## 项目结构
AgenticCPS 采用前后端分离、多模块聚合的工程组织方式，后端以 Spring Boot 3.x 为基础，前端包含 Vue3 管理后台与 UniApp 移动端，同时提供 MCP 协议的 AI 接口层，支撑 AI Agent 直接调用。
...
## 架构总览
AgenticCPS 的后端采用模块化分层：API 定义层、业务实现层、数据访问层、定时任务与 MCP 接口层；前端提供多端体验；openspec 目录承载规范与工作流资产。
...
```mermaid
flowchart TD
Cfg["application-local.yaml"] --> DS["数据源配置"]
Cfg --> RD["Redis 配置"]
Cfg --> MCP["MCP 服务配置"]
Cfg --> Keys["平台 API Key"]
DS --> DB["MySQL"]
...
- 外部依赖
  - Spring AI 与 MCP 协议支持 AI 集成；Flowable 工作流引擎；Redis/Quartz 等基础设施。
- 规范驱动
...
## 故障排查指南
- 环境与依赖
  - 确认 JDK、MySQL、Redis、Maven、Node.js 版本满足要求。
- 启动与配置
...
## 结论
AgenticCPS 通过“规范化工作流 + 低代码 + MCP 协议 + AI 自主编程”的组合拳，实现了从需求到交付的全链路自动化与质量可控。四大核心要素（Specs/Plans/AI Agents/Skills）确保 AI 输出始终遵循既定规范与验收标准，避免“AI 乱写代码”。借助 MCP 工具与平台适配器，系统具备强大的扩展能力与即插即用的 AI 接入能力，真正实现“说一句话就上线”的敏捷交付。
